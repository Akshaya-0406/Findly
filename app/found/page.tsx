...
import { ClipboardCheck, PlusCircle, ChevronLeft, ChevronRight } from "lucide-react";
...
function FoundItemsContent() {
  ...
  useEffect(() => {
    ...
    router.replace(queryString ? `/found?${queryString}` : `/found`, { scroll: false });
  }, [filters, page, router]);
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-accent-50 border border-accent-100 rounded-full text-xs font-semibold text-accent-700">
              ...
              <span>Recovered Feed</span>
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-neutral-900 tracking-tight">
              Found Items
            </h1>
            <p className="text-sm text-neutral-500 max-w-xl">
              Something you found could mean everything to someone. Browse reported findings, identify descriptions, and claim ownership.
            </p>
          </div>
        ...
        <div className="space-y-6">
          ...
          {isLoading ? (
            ...
            <ItemGrid
              items={items}
              emptyTitle="No found items have been reported yet."
              emptyDescription="No found item matches your active filter criteria."
...
export default function FoundPage() {
  return (
    <Suspense fallback={<div className="py-12 text-center text-sm text-neutral-400">Loading found items...</div>}>
      <FoundItemsContent />
    </Suspense>
  );
}
